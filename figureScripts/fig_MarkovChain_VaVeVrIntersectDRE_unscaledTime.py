...
ax1.set_xlim(0,iMax)
ax1.set_ylim(0,4.0e-5)    # 2,5e04 ~ 1.5*max([max(va_i),max(vr_i)])

ax1.set_xticks([100*i for i in range(0,iMax/100+1)])
ax1.set_xticklabels(["" for i in range(0,iMax/100)],fontsize=16)
...
# axes and label adjustements
ax2.set_xlim(0,iMax)
ax2.set_ylim(0,4.0e-5)       # 1.5*max([max(va_i),max(vr_i)])

ax2.set_xticks([100*i for i in range(0,iMax/100+1)])
ax2.set_xticklabels([str(100*i) for i in range(0,iMax/100+1)],fontsize=16)
...
# save figure
plt.tight_layout()
 fig1.savefig('figures/fig_MChain_VaVeVrIntersectionDRE_unscaledTime.pdf')