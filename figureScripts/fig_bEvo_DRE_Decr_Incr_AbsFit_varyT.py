...
#ax2.plot(state1_i,ve1_i,color="black",linewidth=2,label=r'$v_e$')
#ax2.plot(eq_y1i,va1_i,color="green",linewidth=2,label=r'$v_{a,1}$')
#ax2.plot(eq_y1i,vr1_i,'-.',color="green",linewidth=2,label=r'$v_{r,1}$')
scaleFactor = 1e4
...
xBack = 2
# ax2.set_xlim(75,25*(xTickMax-1-xBack))    
# ax2.set_xticks([10*i for i in range(11,14)])
# ax2.set_xticklabels([str(10*i) for i in range(11,14)],fontsize=16)
# ax2.set_xlim(110,130)
ax2.set_yticks([0.02*i for i in range(0,4)])
ax2.set_yticklabels([str(2*i/10.0) for i in range(0,4)],fontsize=16)
ax2.set_ylim(0,0.06)    # 2,5e04 ~ 1.5*max([max(va_i),max(vr_i)])
...
ax2.text(110.5,0.056,r'(B)', fontsize = 22)            

# custom_lines = [Line2D([0], [0], color=myColors[ii], lw=2) for ii in range(len(mcModels))]
# ax2.legend(custom_lines,[ T_vals_strLgd[ii] for ii in T_select],fontsize=14)
