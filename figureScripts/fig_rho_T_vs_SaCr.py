...
#varParam = 'UaMax'
#P_vals = np.logspace(-7,-4,num=20)

#varParam = 'sa'
#P_vals = np.logspace(-3,-1,num=18)

#varParam = 'cr'
#P_vals = np.logspace(-3,-1,num=18)

#varParam = 'Ur'
#P_vals = np.logspace(-7,-4,num=18)

varParam = 'b'
P_vals = np.linspace(1.8,3.2,num=18)

T_ARRY, P_ARRY = np.meshgrid(T_vals, P_vals)
...
for ii in range(int(T_ARRY.shape[0])):
    for jj in range(int(T_ARRY.shape[1])):
        ...
        paramsTemp[varParam] = P_ARRY[ii,jj]
...
cp = ax1.contourf(np.log10(T_ARRY), P_ARRY, RHO_ARRY, levels = myLvls)
cpl = ax1.contour(np.log10(T_ARRY), P_ARRY, RHO_ARRY,colors='k',levels = myLineLvls)
