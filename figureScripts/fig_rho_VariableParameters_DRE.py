...
def get_contourPlot_arrayData():
    ...
    varParam2 = 'alpha'
    P2_vals = 1-np.logspace(-3,-1,num=21)
    ...
    for ii in range(int(P1_ARRY.shape[0])):
        for jj in range(int(P2_ARRY.shape[1])):
            ...
            [dMax,di,iExt] = myfun.get_absoluteFitnessClassesDRE(paramsTemp['b'],paramsTemp['dOpt'],paramsTemp['alpha'],iMax)
            
            [state_i,Ua_i,Ur_i,eq_yi,eq_Ni,sr_i,sa_i] = myfun.get_MChainPopParametersDRE(paramsTemp,di,iExt,yi_option)        
            ...
            [state_i,Ua_i,Ur_i,eq_yi,eq_Ni,sr_i,sa_i,va_i,vr_i,ve_i] = \
                            myfun.get_MChainEvoParametersDRE(paramsTemp,di,iExt,pFixAbs_i,pFixRel_i,yi_option)
...
X_ARRY = np.log10(P1_ARRY)
Y_ARRY = (dMax-paramsTemp['dOpt'])*(1-P2_ARRY)
...
cp = ax1.contourf(X_ARRY, Y_ARRY, RHO_ARRY, levels = myLvls)
cpl = ax1.contour(X_ARRY, Y_ARRY, RHO_ARRY, colors='k',levels = myLineLvls)

ax1.clabel(cpl, fmt='%2.1f', colors='k', fontsize=11)
fig.colorbar(cp) # Add a colorbar to a plot
ax1.set_title(r'$\rho$ Contour Plot')
ax1.set_xlabel('log10 '+varParam1)
ax1.set_ylabel('log10 sa1')
plt.show()
